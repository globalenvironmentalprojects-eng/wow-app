...
function getActiveTasting(qrCode) {
  const qr = DB.qr_codes.find(q => q.code === qrCode);
  if (!qr) return null;
  const tasting = DB.tastings.find(t => t.qrId === qr.id && t.active);
  if (!tasting) return null;
  const wine = DB.wines.find(w => w.id === tasting.wineId);
  // Use tasting-specific steps or fall back to DEFAULT_STEPS
  const steps = tasting.steps || DEFAULT_STEPS;
  return { tasting, wine, qr, steps };
}
...
function SurveyApp({ qrCode = "abc123", onGoProfile }) {
  ...
  const submit = () => { setLoading(true); setTimeout(() => { setLoading(false); setDone(true); }, 1100); };
...
function AdminPanel({ onLogout }) {
  ...
  const addWine = () => {
    if(!nw.name)return;
    setWines(p=>[...p,{...nw,id:"w"+Date.now(),optionsDenomination:[nw.denomination,"Rioja","Ribera del Duero"],optionsName:[nw.name,"Opción 2","Opción 3"]}]);
    ...
    <div className="admin-shell">
        ...
        <>
          ...
          <div className="tbox"><table>
            ...
            <tbody>{ratings.map(r=>{
              ...
              return(<tr key={r.id}><td><strong>{w?.name||"—"}</strong></td><td style={{color:"var(--text-muted)",fontSize:12}}>{t?.event||"—"}</td><td style={{color:"var(--neon)"}}>{("★").repeat(r.answers?.stars||0)}</td><td>{r.answers?.denomination||"—"}</td><td><span className={`tag ${calcCorrect(r.answers||{},t?.steps||DEFAULT_STEPS,w||{}).correct>0?"t-ok":"t-bad"}`}>{calcCorrect(r.answers||{},t?.steps||DEFAULT_STEPS,w||{}).correct}/{calcCorrect(r.answers||{},t?.steps||DEFAULT_STEPS,w||{}).total}</span></td><td style={{fontSize:11,color:"var(--text-muted)"}}>{r.date}</td></tr>);
            })}</tbody>
          </table></div>
        </>
      )}
      ...
      {tab==="qr"&&(
        <>
          ...
          {configuringQr && (
            <SessionConfigModal
              qr={DB.qr_codes.find(q=>q.id===configuringQr)}
              wines={wines}
              denominations={DB.denominations}
              onClose={()=>setConfiguringQr(null)}
              onSave={(qrId, config)=>{
                setTastings(p=>[
                  ...p.map(t=>t.qrId===qrId?{...t,active:false}:t),
                  {
                    id:"t"+Date.now(), qrId, wineId: config.wineId,
